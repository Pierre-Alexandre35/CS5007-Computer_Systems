...
//Generate a cache friendly data access pattern.
void friendlyTrace(int size){
	FILE* fp = fopen("friendly.txt","w");

	for(int i = 0 ; i < size ; i ++){
		 char* bin = intToBinaryString(0,BITWIDTH);
                fprintf(fp,"%s\n",bin);
                free(bin);
        }
...
//Generate a cache unfriendly data access pattern.
void unfriendlyTrace(int size){
    FILE* fp = fopen("unfriendly.txt","w");
        
            for(int i = 0 ; i < size ; i ++){
		   char* bin1 = intToBinaryString((rand()%size),BITWIDTH);
                   char* bin2 = intToBinaryString((rand()%size),BITWIDTH);
                   char* bin3 = intToBinaryString((rand()%size),BITWIDTH);
                   char* bin4 = intToBinaryString((rand()%size),BITWIDTH);
                   char* bin5 = intToBinaryString((rand()%size),BITWIDTH);
		     free(bin1);
                                }   
                                        fclose(fp);
            }
