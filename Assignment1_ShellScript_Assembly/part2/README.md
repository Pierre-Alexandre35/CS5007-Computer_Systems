...
1. What was the bug?
 SIGSEV error which is caused by either running an uninitialized pointer, deferencing a NULL pointer or trying to access memory that we do not really have or that has been deallocated. 
In our program, register rsp has not initialized which caused the error.
2. What line did the bug occur?
The bug occured at the line number 15.


3. What is the fix for the bug?
We need to initialize the register and malloc the memory.
...
Pro:
1. Help to find errors and bugs during the early stages because it is fast and easy to implement. 

Con:
1. We may get some trouble with the run-time because automated tools can not pintpoint weak points. 
...
Pro:
1. Great to find weak aeras in a run time environment.

Con:
1. It will take much more time to find and fix the error/bug.
