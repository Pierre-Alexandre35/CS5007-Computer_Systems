...
I think if a certain painter trapped another painter it has no where to go, because it can only paint where it’s white so that painter is starved for canvas space. Therefore, I think the best way to handle that case in my program is to select initial points at strategic locations. For example, on my program, 50 artists are painting and for some of them, the starting painting point is very close since it is random. So I think we can implement an algorith to give a minimum distance between each painter starting point. 
...
I have some difficulties to use vim to I watched few tutorials on Youtube to have a better knowledge.

- To delete multiple lines in VIM : startingLine, endingLine d such as "3,8d" where d stands for delete.
- To delete multiple lines in VIM : 0,$d
- To quit the program without write (unlinke wq) : q!
- To search for a spefific word : /"word" then "*" to move forward or # to more backward
- To search and remplace : startingLine, endingLine + s / oldCharacter / newCharacter / g such as "12,18s/morning/night/g". In that case, we are going to remplace every "morning" by "night" from lines 12 to 18 included. 
- Go the most recent edited position : "g;"
- Go to the end of a line: "$"
- Go to the next word in the line: "w"
- Undo since the last savinf : "earlier numberf" such as "earlier 2f" to undo the last 2 saved changes. 
- Undo current line : "u"
- Duplicate a line: "yy" on the current line to copy the line then we move the cursor on the desired location and press "p"
