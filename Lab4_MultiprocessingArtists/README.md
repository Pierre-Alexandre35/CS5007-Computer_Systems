...
*Each line corresponds to a machine and lists how many cpus it has.* 
...
I think we should free memory in the child process before it exits.
...
*vfork() suspends the parent process untill the child process terminated,
while for fork(), the child and parent run at the same time.
...
I think there is no need to use locks because each thread work on different row individually, so there is no issue like race condition.
